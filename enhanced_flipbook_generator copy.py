...
def generar_flipbook(df, output="output/flipbook.html"):
    ...
    # --- Página de estadísticas PREMIUM ---
    items_deposito = ''.join([
        f'<div class="stat-item"><span class="stat-item-label">{k}</span><span class="stat-item-value">{v}</span></div>'
        for k, v in estadisticas['por_deposito'].items()
    ])
    
    # Calcular porcentajes y grados para la gráfica de torta de gerencias
    total_gerencias = sum(estadisticas['por_gerencia'].values())
    gerencias_list = list(estadisticas['por_gerencia'].items())
    
    # Calcular grados para cada segmento
    deg_acumulado = 0
    grados = []
    for nombre, valor in gerencias_list:
        porcentaje = (valor / total_gerencias) * 100
        deg = (valor / total_gerencias) * 360
        grados.append((deg_acumulado, deg_acumulado + deg, porcentaje))
        deg_acumulado += deg
    
    # Crear estilos inline para el conic-gradient
    colores_gerencias = ['#FF0000', '#0A2D82', 'rgba(255,255,255,0.2)', 'rgba(188,189,188,0.5)']
    gradient_stops = []
    for i, (nombre, valor) in enumerate(gerencias_list):
        color = colores_gerencias[i % len(colores_gerencias)]
        deg_inicio = grados[i][0]
        deg_fin = grados[i][1]
        gradient_stops.append(f"{color} {deg_inicio}deg {deg_fin}deg")
    
    pie_gradient = f"conic-gradient({', '.join(gradient_stops)})"
    
    # Crear items de leyenda
    items_gerencia_legend = ''.join([
        f'''<div class="legend-item">
            <div class="legend-color color-{i+1}"></div>
            <span class="legend-label">{nombre}</span>
            <span class="legend-value">{valor}</span>
        </div>'''
        for i, (nombre, valor) in enumerate(gerencias_list)
    ])
    ...
    <div class="double stats-premium">
        <div class="stats-container">
            ...
            <div class="stats-content-grid">
                ...
                <!-- Card 2: Por Gerencia CON GRÁFICA DE TORTA -->
                <div class="stat-glass-card" style="grid-column: span 2;">
                    <div class="stat-card-title">Distribución por Gerencia</div>
                    <div class="pie-chart-container">
                        <div class="pie-chart" style="background: {pie_gradient};"></div>
                        <div class="pie-chart-legend">
                            {items_gerencia_legend}
                        </div>
                    </div>
                </div>
                
                <!-- Card 3: Por Estado (Wide) -->
    ...
    logger.info(f"📈 Estadísticas con gráfica de torta incluida")
